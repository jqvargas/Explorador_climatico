...
TABLES <- c("estacion", "estacion_atributos_extra", "fuente", "modelo", "variable",
...
for (tbl in TABLES) {
  tryCatch({
    ...
    cat(sprintf("%-25s %12s %12s %6s\n", tbl, format(src_n, big.mark=","), format(dst_n, big.mark=","), if (src_n == dst_n) "OK" else "DIFF"))
  }, error = function(e) cat(sprintf("%-25s %12s %12s %6s\n", tbl, "error", "error", "ERR")))
}
